  ...
  implements GGButtonListener
{
  ...
  private DieOperation dieOperation = new DieOperation(this);
  ...
  private int getDieValue() {
    ...
    int currentIndex = (nbRolls / gp.getNumberOfPlayers()) * numberOfDice + dieOperation.getCurNumOfDice();
    int playerIndex = nbRolls % gp.getNumberOfPlayers();
    if (dieValues.get(playerIndex).size() > currentIndex) {
      return dieValues.get(playerIndex).get(currentIndex);
    }
    return RANDOM_ROLL_TAG;
  }
  ...
  void prepareRoll(int currentIndex)
  {
    ...
    else
    {
      ...
      //Check if two puppet at the same location before switching the puppet
      if (isAtSameLocation(gp.getCurrentPuppetIndex())){
        gp.getAllPuppets().get((gp.getCurrentPuppetIndex() + 1) % gp.getNumberOfPlayers()).go(-1);
  ...
  void prepareBeforeRoll() {
    if(dieOperation.getSizeOfDice() == numberOfDice) {
  ...
  private void roll(int rollNumber)
  {
    ...
    dieOperation.addDie(nb);
    addActor(dieOperation.getLastDie(), dieBoardLocation);
  }
  ...
  private boolean isAtSameLocation(int currentIndex){
    int opponent = (currentIndex + 1) % gp.getNumberOfPlayers();
    if (gp.getAllPuppets().get(currentIndex).getCellIndex() == gp.getAllPuppets().get(opponent).getCellIndex()){
      return true;
    }
    return false;
  }
